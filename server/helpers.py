...
def deriveStats(interaction_metadata, fd_metadata, h_space, study_metrics, dirty_dataset, clean_dataset, target_fd):
    ...
    for i in iters:
        ...
        max_h_lhs = set(max_h.split(' => ')[0][1:-1].aplit(', '))
        max_h_rhs = set(max_h.split(' => ')[1].split(', '))
        for h in h_space:
            successes = 0
            failures = 0

            h_lhs = set(h['cfd'].split(' => ')[0][1:-1].aplit(', '))
            ...
            if fd != max_h and (max_h == 'Not Sure' or fd_m['conf'] > fd_metadata[max_h]['conf_history'][-1]['value']):
                max_h = fd
