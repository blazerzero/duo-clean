...
from analyze import sHeuristicSetRelation
...
def reinforceTuplesBasedOnDependencies(data, project_id, current_iter, is_new_feedback, project_info):
    ...
    for cfd, cfd_m in cfd_metadata.items():
        ...
        complexity_bias = sHeuristicSetRelation(cfd, all_cfds)
...
def buildSample(data, sample_size, project_id, sampling_method, current_iter):
    ...
    for cfd, cfd_m in cfd_metadata.items():
        ...
        # I(y in supp(h))
        if cfd not in modeling_metadata['y_supp_h'].keys():     # cfd was just discovered in this iteration
            modeling_metadata['y_supp_h'][cfd] = dict()
            for y in data.index:
                modeling_metadata['y_supp_h'][cfd][y] = 1 if y in cfd_m['cover'] else 0     # I(y in supp(h)) = 1 if y is supported by the cfd, otherwise 0
