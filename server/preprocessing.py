...
if __name__ == '__main__':
    ...
    for s_id, scenario in tqdm(scenarios.items()):
        ...
        if int(s_id) <= 4:
            ...
            clean_process = sp.Popen(['./data/cfddiscovery/CFDD', scenario['clean_dataset'], str(len(data.index)), '0.7', '2'], stdout=sp.PIPE, stderr=sp.PIPE, env={'LANG': 'C++'})
        ...
        if clean_process.returncode == 0:
            ...
            clean_fds = [c for c in json.loads(output, strict=False)['cfds'] if '=' not in c['cfd'].split(' => ')[0] and '=' not in c['cfd'].split(' => ')[1] and c['cfd'].split(' => ')[0] != '()']
            # output = res[0].decode('latin_1')
            # fds = helpers.parseOutputPYRO(output)
            # scenario['hypothesis_space'] = fds
        else:
